...
\end{equation}
\paragraph{Encoding and decoding functions:} \stavros{figure out where to place the discussion about f,g and g's capacity}
...
$e_{\p{i},r_j}=\norm{D_{\p{i},r_i}^I - \tilde{D}_{\p{i},r_j}^I}_2^2 = e_{ij}$, given the functions $f,g$.
...
Examples of simplified medial axes are illustrated in~\reffig{fig:reconstruction}.
...
\subsection{Disk Cost Computation}
...
\subsection{Dealing with textured images}
...
\subsection{Reconstructing the image from AMAT}
...
Note that our algorithm does not return a probability-like skeleton map, so we cannot 
plot precision-recall curves by varying a score threshold.
