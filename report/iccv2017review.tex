...
The medial axis transform (MAT) is a powerful abstraction of 2D and 3D shapes 
that has been successfully used in shape recognition, matching and retrieval. 
One of its most important traits is that it provides a very compact representation that can
be used to accurately reconstruct the input shape~\cite{tam2003shape,<li2015q}; 
this makes the MAT very useful for reducing complexity and computational demands for various algorithms.
...
Previous works have mostly attacked the problem of \emph{medial point detection}: 
determining \emph{locations} of medial points but not their exact scale~\cite{tsogkas2012learning,shen2016object}.
...
focuses on detecting \emph{object} skeletons, ignoring medial axes of background structures.
...
In this paper we frame the MAT for natural images as a 
...
First, AMAT encompasses all the fundamental features of its  binary counterpart. 
Our definition is centered around the invertibility property of the MAT, and elicits 
...
%It does not use machine learning either, which liberates us from the need of gathering
