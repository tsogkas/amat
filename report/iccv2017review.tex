...
\label{alg:greedy}
	\begin{algorithmic}[1]
		\Procedure{AMAT}{$I,\set{R},f,g$} \Comment{Image $I$ has $N$ pixels and domain.}
		\State Initialize solution as $M_0 = \{(\p{1},r_1),\ldots,(\p{N},r_N)\}$.
		\State Compute $f\left( D_I(\p{},r) \right),\  \forall D_I(\p{},r) \in I$.
		\ForAll{$\p{i},\p{j} \in ,\ r_i,r_j \in $}  \Comment{Remove non-maximal disks.}
			\If{$e\left( f\left( D_I(\p{i},r_i) \right), f\left( D_I(\p{j},r_j) \right) \right) < \epsilon$ 
			and $r_i < r_j$} 
				$M_0 \gets M_0-\{(\p{i},r_i)\}$.
			\EndIf
		\EndFor
		\State $M = \argmin_{(\p{},r)\in M_0}{d\left( \bigcup_{i=1}^n \bar D_I(\p{i},r_i), I \right)}$ \Comment{Select subset that optimizes reconstruction quality.}
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
