...
\stavros{Should we change that with Color-MAT (CMAT)?}
...
\caption{\textbf{Top:} Input image and segmentation from BSDS500 with overlayed medial axes for each segment (green)
 ...
 input segments. \textbf{Bottom:} Similarly, the Appearance-MAT carries enough information to reconstruct the \emph{input image}.}
\label{fig:teaser}
\end{figure}
...
because of its increased robustness to viewpoint changes and its close relationship to salient structures, such as
object parts.
...
geons~\cite{biederman1987recognition}, shock graphs~\cite{siddiqi1999shock}.

Fundamental notions of local symmetry were introduced by Blum in the context 
...
\stavros{Should these go in the introduction or here?: "[The only criterion that drives medial point detection in our case is ...]" 
and also talk about the fact that we encode scale and appearance information}
...
\stavros{We could potentially use a given ``budget'' of points, $m$}
...
\stavros{We will add discussion about how to obtain the reconstruction in a previous section}
...
All results are reported in~\reftab{tab:experiments:reconstruction}.
