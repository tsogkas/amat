...
Our methods significantly outperforms the baselines in terms of reconstruction quality, while attaining good compression ratio.
...
Now let $g:\R^K \rightarrow \R^3$ be a function that maps $\f{\p{},r}$ back to a disk patch $\tilde{D}_{\p{},r}^I$: 
...
$\set{D} = \{D_1,D_2,\ldots,D_k\} \subseteq \set{X}$. 
...
touching the shape boundary at exactly two points.\stavros{exactly two or more than two}
...
Using a criterion such as MSE if not effective;~\reffig{fig:google} shows that naively selecting a disk with 
